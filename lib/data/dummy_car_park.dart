import '../models/carPark.dart';

const DUMMY_CAR_PARKS = const [
  CarPark(id: "001asd", name: "Colombo city center", avaliableSlot: 18),
  CarPark(id: "002dbj", name: "One Galle Face", avaliableSlot: 24),
  CarPark(id: "003hkd", name: "Marino Mall", avaliableSlot: 26),
  CarPark(id: "005vhss", name: "Majestic City", avaliableSlot: 0),
  CarPark(id: "001asd", name: "Colombo city center", avaliableSlot: 18),
  CarPark(id: "002dbj", name: "One Galle Face", avaliableSlot: 24),
  CarPark(id: "003hkd", name: "Marino Mall", avaliableSlot: 26),
  CarPark(id: "005vhss", name: "Majestic City", avaliableSlot: 20),
  CarPark(id: "003hkd", name: "Marino Mall", avaliableSlot: 26),
  CarPark(id: "005vhss", name: "Majestic City", avaliableSlot: 12),
  CarPark(id: "001asd", name: "Colombo city center", avaliableSlot: 0),
  CarPark(id: "002dbj", name: "One Galle Face", avaliableSlot: 24),
  CarPark(id: "003hkd", name: "Marino Mall", avaliableSlot: 0),
  CarPark(id: "005vhss", name: "Majestic City", avaliableSlot: 20),
];
